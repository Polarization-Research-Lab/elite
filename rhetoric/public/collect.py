...
for year in years:
    ...
    joined_data = year_data.join(legislators, year_data['bioguide_id'] == legislators['bioguide_id'])[
    ...
    final_data = joined_data.join(tweets, joined_data['source_id'] == tweets['id'])[
        joined_data,
    ...
    final_data = final_data.mutate(
        ...
        text = ibis.case()
            .when(final_data['source'] == 'tweets', ibis.literal(''))
...
pd.DataFrame(conn.raw_sql(meta_data_query.format(table = 'classifications'))).query('COLUMN_NAME not in ["id", "dictionary", "errors", "valence"]').to_csv('.tmp/communication-meta.csv', index = None) # <-- save schema to csv
...
# Get Schemas
## Pull and format
ideology = pd.DataFrame(conn.raw_sql(meta_data_query.format(table = 'ideology'))).query('COLUMN_NAME != "id"')
efficacy = pd.DataFrame(conn.raw_sql(meta_data_query.format(table = 'efficacy'))).query('COLUMN_NAME != "id"').query('COLUMN_NAME != "bioguide_id"')
attendance = pd.DataFrame(conn.raw_sql(meta_data_query.format(table = 'attendance'))).query('COLUMN_NAME != "id"').query('COLUMN_NAME != "bioguide_id"')
money = pd.DataFrame(conn.raw_sql(meta_data_query.format(table = 'money'))).query('COLUMN_NAME != "id"').query('COLUMN_NAME != "bioguide_id"')
rhetoric = pd.DataFrame(conn.raw_sql(meta_data_query.format(table = 'rhetoric'))).query('COLUMN_NAME != "id"').query('COLUMN_NAME != "bioguide_id"')
...
# Add to Download_Counts table
with dataset.connect(params) as dbx:
    for path in paths:
        dbx['download_counts'].upsert(
            {'file': path},
            'file'
        )
